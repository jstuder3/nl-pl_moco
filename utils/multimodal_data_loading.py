...
def convert_examples_to_features(examples, docs_tokenizer, code_tokenizer, stage=None):
    features = []
    counter=0
    for example_index, example in enumerate(examples):
        ...
        # code
        source_tokenized=code_tokenizer(example.source, truncation=True, padding="max_length")
        # nl
        target_tokenized=docs_tokenizer(example.target, truncation=True, padding="max_length")
        ...
        if False and example_index < 3:
            if stage == 'train':
                ...
                logger.info("source_tokens: {}".format([x.replace('\u0120', '_') for x in code_tokenizer.decode(source_tokenized["input_ids"])]))
                ...
                logger.info("target_tokens: {}".format([x.replace('\u0120', '_') for x in docs_tokenizer.decode(target_tokenized["input_ids"])]))
...
def generateDataLoader(language, split, docs_tokenizer, code_tokenizer, args, shuffle=False, augment=False, num_workers=0):
    ...
    features = convert_examples_to_features(examples, docs_tokenizer, code_tokenizer, stage=split)
    ...
    dataloader = DataLoader(data, batch_size=int(args.effective_batch_size/args.num_gpus), drop_last=True, shuffle=shuffle, num_workers=num_workers, pin_memory=True)
