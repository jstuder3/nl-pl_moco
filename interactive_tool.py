...
parser.add_argument("--do_adv_test", default=False)
...
if gui_mode:
    ...
    app.go()
#meant for fast precomputation of the embeddings on the server
else:
    subsets = {"Train": True, "Validation": True, "Test": True}
    language="php"
    checkpoint = "Oct22_05_46_05-xMoCo-php"
    query = "Print the current time"
    num_samples = 1
    ...
    for key in subsets:
        if subsets[key]:
            ...
            try:
                ...
                print(f"No cached tensors found. Generating embeddings...")
                if model==None or prev_language!=language:
                    model=xMoCoModelPTL.load_from_checkpoint(f"checkpoints/{checkpoint}.ckpt")#{language}.ckpt") # note: change checkpoint to the desired language
                    prev_language=language
